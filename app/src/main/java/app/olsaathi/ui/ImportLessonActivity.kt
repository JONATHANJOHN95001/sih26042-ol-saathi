...
class ImportLessonActivity : AppCompatActivity() {
    ...
    private val openLauncher = registerForActivityResult(
        ...
        if (uri != null) {
            ...
            currentUri = uri
            title = displayName(uri) ?: "Imported lesson"
            importPdf(uri)
        }
    }
    ...
    private fun importPdf(uri: Uri) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                var r = LessonPdfImporter(this@ImportLessonActivity).import(uri, forPack)
    ...
    private suspend fun translateMissing(r: ImportResult, forPack: VerifiedContentPack): ImportResult {
        ...
        if (missing.isEmpty() || !client.isConfigured || !NetworkGuard.isOnline(this)) return r
    ...
    private fun showResult(r: ImportResult) {
        ...
        if (r.lines.isEmpty()) {
            binding.textStatus.visibility = View.VISIBLE
            binding.textStatus.text = "This is a scanned or image-only PDF. No text could be extracted."
            return
        }
        ...
        val fromPack = r.lines.count { it.isCovered && !it.live }
        ...
        binding.textCoverage.text = buildString {
            ...
            append(".\nWorksheets and flashcards are tagged with NIPUN Bharat outcomes.")
        }
    ...
    inner class ImportedLineAdapter(
        ...
        override fun onBindViewHolder(holder: VH, position: Int) {
            ...
            if (t != null) {
                ...
                val label = if (line.live) "Live machine translation · Bhashini" else "Offline pack"
