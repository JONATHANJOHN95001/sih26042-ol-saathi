...
class HindiSpeechInput(
    ...
    /**
     * Whether to ask the recogniser for offline Hindi. Starts true, because
     * the classroom is offline, and drops to false for the rest of this
     * session the first time the device says it has no offline Hindi
     * (errors 12 and 13), so the next attempt goes to the online recogniser
     * instead of failing the same way every time. Seen on a OnePlus Nord 5
     * whose Google recogniser had no Hindi pack downloaded.
     */
    ...
    private fun createListener() = object : RecognitionListener {
        ...
        override fun onError(error: Int) {
            // No offline Hindi on this device: try once more through the
            // online recogniser before telling the teacher anything. If there
            // is no network either, that attempt fails with a network error,
            // which is reported normally below.
            if ((error == 12 || error == 13) && preferOffline) {
                Log.w(TAG, "Speech error $error with offline Hindi; retrying online")
                preferOffline = false
                begin()
                return
            }
