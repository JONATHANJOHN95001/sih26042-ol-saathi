...
PROVENANCE = {
    ...
    'parler': 'Synthesised by AI4Bharat Indic Parler-TTS',
}
...
def main():
    ...
    with io.open(PACK, 'w', encoding='utf-8', newline='\n') as fh:
        ...
        fh.write('\n')
    # The language bar reads its "has audio" note from packs.json, so keep the
    # count there in step with what was just written.
    from pack_manifest import refresh_audio_counts
    for code, old, new in refresh_audio_counts():
