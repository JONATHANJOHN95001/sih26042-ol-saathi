...
def main() -> int:
    ...
    # Every pack, not just Santali. Build-time Bhashini audio lives per language
    # under pack/audio/<lang>/, and a check that only read pack.sat.json would
    # pass while Tamil pointed at files that do not exist.
    pack_audio_paths: set[str] = set()
    for pack_path in sorted((ASSETS / 'pack').glob('pack.*.json')):
        with open(pack_path, 'r', encoding='utf-8') as f:
            for entry in json.load(f).get('entries', {}).values():
                audio_rel = entry.get('audio')
                if audio_rel:
                    pack_audio_paths.add(audio_rel)

    audio_files = (sorted(audio_dir.rglob('*.wav')) + sorted(audio_dir.rglob('*.ogg'))
                   if audio_dir.exists() else [])
    if audio_dir.exists():
        files_on_disk: set[str] = set()

        for clip in audio_files:
            rel_path = 'pack/audio/' + clip.relative_to(audio_dir).as_posix()
            files_on_disk.add(rel_path)
            data = clip.read_bytes()
            checks_ran += 1

            if clip.suffix == '.ogg':
                # The platform decodes OGG, not this script, so check what can be
                # checked without a codec: the container signature, a size that
                # could hold a spoken line, and that some pack uses the file.
                if data[:4] != b'OggS':
                    failures.append((rel_path, 'not an OGG container'))
                    continue
                if len(data) < 2000:
                    failures.append((rel_path, 'only %d bytes, too small for a spoken line' % len(data)))
                    continue
                if rel_path not in pack_audio_paths:
                    failures.append((rel_path, 'file exists on disk but no pack entry references it'))
                    continue
                print('  ok    audio  %-38s %10d bytes  OGG' % (rel_path, len(data)))
                continue
        ...
        # No audio directory: check that no pack references audio files either
        if pack_audio_paths:
            failures.append(('pack/audio/', 'audio directory missing but pack references audio files'))

    # ── manifest agrees with the packs ────────────────────────────────────
    # The language bar says "No audio in Tamil yet" from packs.json, not from
    # the pack, so a stale count is a false statement on screen even when every
    # clip is real. tools/pack_manifest.py fixes it.
    sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
    import pack_manifest
    manifest_drift = pack_manifest.drift(str(ASSETS / 'pack'))
    for code, said, has in manifest_drift:
        failures.append(('pack/packs.json',
                         '%s: says %d entries have audio, the pack has %d. Run tools/pack_manifest.py' % (code, said, has)))
    if not manifest_drift:
        print('  ok    manifest packs.json audio counts match every pack')

    for rel, why in failures:
        print('  FAIL         %-38s %s' % (rel, why))

    total_fonts = len(FONTS)
    audio_checked = len(audio_files)
    total = total_fonts + audio_checked + 1
